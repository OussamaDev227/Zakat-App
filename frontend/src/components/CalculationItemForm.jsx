/**
 * Calculation Item Form Component
 * 
 * Form to add/edit items within a calculation
 * Similar to FinancialItemForm but optimized for inline use
 * 
...
export default function CalculationItemForm({ calculationId, item = null, onSubmit, onCancel }) {
  ...
  useEffect(() => {
    if (formData.category === 'ASSET') {
      ...
      setFormData(prev => ({ ...prev, liability_code: '', asset_type: null, equity_code: '' }));
    } else {
      setFormData(prev => ({ ...prev, equity_code: '', asset_type: null, liability_code: '' }));
    }
  }, [formData.category]);
